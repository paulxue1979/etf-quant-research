...
export function BacktestReportCharts({ report, series, markerReport = null, markerLoading = false, markerError = null, onMarkerTypesChange, loading = false, error = null }: BacktestReportChartsProps) {
  ...
  return <section className="financial-charts" aria-label="Core financial charts">
    ...
    <fieldset className="marker-toolbar" aria-label="Event marker filters">
      ...
      <div className="marker-toolbar-layout">
        <div className="marker-toolbar-section marker-category-section" role="group" aria-label="Event categories"><span className="marker-toolbar-label">Event categories</span><div className="marker-filter-grid">{MARKER_FILTERS.map((item) => <label className="marker-toggle" key={item.type}><input type="checkbox" checked={markerTypes.has(item.type)} onChange={() => toggleMarkerType(item.type)} /><span>{item.label}</span></label>)}</div></div>
        <div className="marker-toolbar-section" role="group" aria-label="Marker density"><span className="marker-toolbar-label">Density</span><label className="marker-toggle"><input type="checkbox" checked={majorOnly} onChange={(event) => setMajorOnly(event.target.checked)} /><span>Major only</span></label></div>
        <div className="marker-toolbar-section" role="group" aria-label="Display threshold"><label className="marker-threshold-label" htmlFor="major-marker-threshold"><span className="marker-toolbar-label">Display threshold</span><input id="major-marker-threshold" aria-label="Major marker threshold" type="number" min="0" max="1" step="0.01" value={majorThreshold} disabled={!majorOnly} onChange={(event) => setMajorThreshold(Math.min(1, Math.max(0, Number(event.target.value) || 0)))} /></label></div>
        <div className="marker-toolbar-section marker-status" aria-label="Marker filter status"><span className="marker-toolbar-label">Status</span><span role="status">{markerProjection.visibleEventCount} events shown{markerProjection.hiddenEventCount > 0 ? ` · ${markerProjection.hiddenEventCount} hidden by filters` : ""}</span></div>
      </div>
