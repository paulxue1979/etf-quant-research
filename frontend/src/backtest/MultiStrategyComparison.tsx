...
export function MultiStrategyComparison({ comparison }: { comparison: ResearchComparison }) {
  const series = useMemo(() => buildComparisonSeries(comparison), [comparison]);
  ...
  const visible = useMemo(
    () => visibleComparisonSeries(series, hiddenRunIds, focusRunId),
    [focusRunId, hiddenRunIds, series],
  );
  const fullRange = useMemo(() => comparisonFullRange(series), [series]);
  ...
  return <section className="research-comparison" aria-label="Multi-strategy TWR comparison workspace">
    ...
    <section className="panel comparison-workspace">
      <div className="financial-charts-toolbar">
        <div><span className="eyebrow">MULTI-STRATEGY RESEARCH</span><h2>Canonical TWR comparison</h2></div>
      ...
      <div className="comparison-legend-toolbar">
        <div className="comparison-legend" aria-label="Comparison legend">
          {series.map((item) => {
            ...
            ><i style={{ background: item.color }} /><span>{item.label}</span>{item.status !== "available" && <small>{item.reason ?? item.status}</small>}</button>;
          })}
        </div>
      ...
      {blocked
        ...
        : <MultiStrategyTwrChart allSeries={series} hiddenRunIds={hiddenRunIds} focusRunId={focusRunId} visibleSeries={visible} onReady={registerChart} />}
